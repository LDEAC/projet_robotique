...
while robot.step(timestep) != -1:
    ...
    # 1) Lecture capteurs (vraie boucle)
    vals = [0.0] * 7
    for i in range(7):
        vals[i] = ds[i].getValue()

    # 2) Moyennes pondérées (identiques à ton code)
    moy_gauche = (vals[0] + 5*vals[1] + 20*vals[2]) / 26
    moy_droite = (20*vals[2] + 5*vals[3] + vals[4]) / 26
    ...
    # 5) état (basées sur les vitesses)
